...
const SearchScreen = ({navigation,route}) => {
  ...
  axios({
    ...
    url: `${BASE_URL}/search?username=${input}`,
  }).then((response) => {
    let profiles = response.data.users
    setProfile(profiles)
  });
  ...
  return (
    <SafeAreaView style={{ backgroundColor: 'black', flex: 1}}>
      ...
      <>
          <ScrollView>
            <SearchList profile={profile} navigation={navigation} user={user} userInfo={userInfo}></SearchList>
          </ScrollView>
        </>
    </SafeAreaView>
  )
